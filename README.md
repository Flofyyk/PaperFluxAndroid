...
Интерфейс рассчитан на устройства с разной плотностью экрана и системной навигацией: учитываются статус-бар, вырезы, клавиатура и нижняя панель Android. На узких экранах текст переносится, а элементы управления остаются доступными.

Чтобы не нагружать слабые устройства, интерфейс использует системные шрифты, не загружает внешние ресурсы при старте и не перерисовывает экран при неизменной статистике. Журнал отрисовывает только видимую область, а список исключений приложений — только строки в области прокрутки.
